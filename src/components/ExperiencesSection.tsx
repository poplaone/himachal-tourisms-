...
import { Plane, Hotel, Map, UtensilsCrossed, Car, Calendar, Check, X, ArrowRight } from "lucide-react";
...
const InclusionExclusionList = ({
  ...
  return <div className="mb-3">
      ...
      <ul className="space-y-1">
        {items.map((item, idx) => <li key={idx} className="flex items-start text-[11px] md:text-xs">
            {type === "inclusion" ? <Check className="w-3 h-3 mr-1 text-green-400 mt-0.5 flex-shrink-0" /> : <X className="w-3 h-3 mr-1 text-red-400 mt-0.5 flex-shrink-0" />}
...
const TourPackageCard = ({
  ...
  return <Card className="bg-gradient-to-br from-gray-900/90 to-gray-800/50 border-gray-700/30 backdrop-blur-sm overflow-hidden h-full text-white" style={{
    opacity: 0,
    animation: `fade-in 0.7s forwards ease-out ${delay}s`
      ...
      <CardContent className="space-y-4 pt-0">
        ...
        {/* Pricing */}
        {batchPrice && <div className="flex flex-col items-center text-center bg-purple-900/20 rounded-lg p-2 mb-3">
        ...
        {customPricing && <div className="flex justify-between gap-2 mb-3">
            <div className="flex-1 bg-purple-900/20 p-2 rounded-lg text-center">
              ...
              {customPricing.sharing && <div className="text-xs text-gray-300">{customPricing.sharing}</div>}
            </div>
            
            {customPricing.title2 && <div className="flex-1 bg-purple-900/20 p-2 rounded-lg text-center">
                ...
                {customPricing.sharing2 && <div className="text-xs text-gray-300">{customPricing.sharing2}</div>}
              </div>}
          </div>}
        
        {/* Private Tour Pricing */}
        {privateTourPrices && privateTourPrices.twoPersons !== "On Demand Only" && <PriceTable title="For Private Tour" privateTourPrices={privateTourPrices} />}
...
const ExperiencesSection = () => {
  ...
  return <section className="py-16 md:py-24 text-gray-200">
